...
def input_cur(type, length, current_value, time_step,input_features): 
	"""
	...
	if (type == "webb_advance"):
		webb_input = torch.zeros(length,input_features)
		mu = 5
		A = current_value
		for i in range(length):
			time = i*time_step #ms
			...
			if 700<=time<1200:
				time = time-700
				webb_input[i,:i]=0.5/0.5*A*math.cos(math.pi/1000*time)+mu
			if 1200<=time<1500:
				time = time-1000
				webb_input[i,:]=mu
			if 1500<=time<2000:
			...
			if 2500<=time<3125:
				time = time-2500
				webb_input[i,:]=0.3/0.5*A*math.sin(-4*math.pi/1000*time)+mu
			if 3125<=time<3500:
				webb_input[i,:]=mu-0.3/0.5*A
			if 3500<=time<3875:
				time = time-3500
				webb_input[i,:]=-0.3/0.5*A*math.cos(4*math.pi/1000*time)+mu
			if 3875<=time<4100:
				webb_input[i,:]=mu
			if 4100<=time<4500:
				time = time-4100
				webb_input[i,:]=0.2/0.5*A*math.sin(8*math.pi/1000*time)+mu
			if 4100<=time<4600:
				time = time-4100
				webb_input[i,:]=0.2/0.5*A*math.sin(8*math.pi/1000*time)+mu
			if 4600<=time<5000:
				time = time-4600
				webb_input[i,:]=0.5/0.5*A*math.sin(2*math.pi/1000*time)+mu
