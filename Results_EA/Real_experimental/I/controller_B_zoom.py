...
all_files =["PID_with_I","SNN"]
colors_files = ["tab:blue","tab:orange"]
labels = ["PID","SNN Controller","Target"]
color_ref = "tab:red"
opacity = 0.8
start = 141
stop_seconds= 143
include_average_line = False
#A: 70-80
#B: 140-150
...
for file_name in all_files:
    ...
    for file in all_files:
        ...
        ax0.plot(time,meas, color = colors_files[ind], alpha= opacity, linewidth=3)

        # ax1.plot(time, u_pd,color = colors_files[ind], alpha= opacity)
        # ax2.plot(time,u_i,color = colors_files[ind], alpha= opacity)
        ax3.plot(time, u_i+u_pd,color = colors_files[ind], alpha= opacity, linewidth=3)
    ...
    if include_average_line:
        ...
        avg_pd = np.clip(avg_pd, -6,6)
        ax0.plot(time,avg_meas, color = colors_files[ind],linewidth =3,alpha=1,zorder=100)
        # ax1.plot(time,avg_pd*0.33, color = colors_files[ind],linewidth =3,alpha=1,zorder=100)
        # ax2.plot(time,avg_i*0.33, color = colors_files[ind],linewidth =3,alpha=1,zorder=100)
...
ax0.legend(lines,labels,loc='lower left', frameon=True, shadow=True,ncols=3)
...
plt.savefig("/home/tim/Documents/plots_papers/controller_B_zoom.pdf")
plt.show()