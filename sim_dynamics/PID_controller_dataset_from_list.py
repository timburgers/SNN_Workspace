...
setpoint_list = [0,0.5,0.5,0.5,0.5]
freq_setpoint = 50 #s

SETPOINT_Z = 0 		# Setpoint of height [m]
Z_INITIAL = 0.5 		# Initial height [m]
...
class Simulation_PID(object):
	def __init__(self):
		...
		with open("configs/config_LIF_DEFAULT.yaml","r") as f:
			...
			self.config["Z_INITIAL"] = Z_INITIAL
	...
	def PID_cycle(self):
		...
		while(self.sim):
			...
			if self.timer%(freq_setpoint*10)==0:
				SETPOINT_Z = setpoint_list[setpoint_counter]
				setpoint_counter +=1
...
def save_data(z,z_ref,error,kpe,kie,kde, T):
	z.shape = [len(z),1]
	...
	np.savetxt("Sim_data/height_control_PID/moving_bias_slow_steps/dataset_" + str(1000)+ ".csv",np.concatenate([z,z_ref,error,kpe,kie,kde,kpe+kde,T],axis=1) , delimiter=',', header= "meas,ref,error,kp,ki,kd,kpd, u")
