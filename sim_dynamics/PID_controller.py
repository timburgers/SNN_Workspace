...
antiWindup = True	# If set to true, no windup will take place when the trust limit is reached in the integration error
...
class Simulation_PID(object):
	def __init__(self):
		global SETPOINT_Z
		SETPOINT_Z = 0 	
		self.states = Drone(Z_INITIAL, DZ_INITIAL, MASS, TIME_STEP, g)
	...
	def PID_cycle(self):
		...
		# save_data(self.z,self.z_ref,self.kpe,self.kde,self.thrst)
...
def save_data(z,z_ref,kpe,kde, T):
	z.shape = [len(z),1]
	z_ref.shape= [len(z_ref),1]
	kpe.shape = [len(kpe),1]
	kde.shape = [len(kde),1]
	T.shape = [len(T),1]
	np.savetxt("D_freq_" + str(SETPOINT_UPDATE_STEP) + "_Min_diff_" + str(MIMIMAL_HEIGHT_CHANGE)+"_Seed_" + str(RANDOM_SEED) + ".csv",np.concatenate([z,z_ref,kpe,kde,T],axis=1),delimiter=',')
...
class PID(object):
	...
	def compute(self, setpoint, measure):
		...
		self.output = self.kp * self.error + self.ki*self.integral_error + self.kd*self.derivative_error + (-g * MASS)
		#print(self.output)
...
# for SETPOINT_UPDATE_STEP in [4,5,6,7,8,9,10]:
