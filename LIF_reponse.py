from SNN_LIF_LI_init import LIF_single_test
import torch
import pandas as pd
...
range_step = [-1,1]
number_of_steps = 10
model = LIF_single_test(None,neurons,weight,bias)
...
if stepwise_response != True:
    ### Get the input and target data
    input_data, _ = get_dataset(dataset_number,sim_time)
else:
    input_data = np.array([])
...
l1_spikes, l1_state, control_output = model(input_data, snn_states,LI_state)
